...
public class TreeMapper implements TreeMappingConstants {
	private static final String propertySimilarity = "similarity";
	...
	private int numOfChangeNodesM = 0, numOfChangeNodesN = 0;
	...
	public int getNumOfChangeNodesM() {
		return numOfChangeNodesM;
	}

	public int getNumOfChangeNodesN() {
		return numOfChangeNodesN;
	}
	...
	public int map(MapAstNode mm1, MapAstNode mm2)
	{
		...
		this.numOfChangeNodesM = mapUnchange(astM, treeM);
		this.numOfChangeNodesN = mapUnchange(astN, treeN);
	...
	private int mapIsomorphicLeaves(ArrayList<ASTNode> leavesM, ArrayList<ASTNode> leavesN)
	{
		...
		while(m < leavesM.size() || n < leavesN.size())
		{
			...
			while(!hasNeighbor && (m < leavesM.size() || n < leavesN.size()))
			{
				...
				{
					leafM = leavesM.get(m);
					int status = (Integer)leafM.getProperty(TreeMappingConstants.propertyStatus);
					if(status == TreeMappingConstants.STATUS_UNCHANGED)
						break;
					m++;
				}
				...
				{
					leafN = leavesN.get(n);
					int status = (Integer)leafN.getProperty(TreeMappingConstants.propertyStatus);
					if(status == TreeMappingConstants.STATUS_UNCHANGED)
						break;
					n++;
				}
				if(m < leavesM.size() && n < leavesN.size())
				{
					...
					if(m > 0 && (Integer)leavesM.get(m-1).getProperty(TreeMappingConstants.propertyStatus) == TreeMappingConstants.STATUS_UNCHANGED)
						hasNeighbor = true;
					...
					if(nodeTypeM != ASTNode.RETURN_STATEMENT && nodeTypeM != ASTNode.BREAK_STATEMENT && nodeTypeM != ASTNode.CONTINUE_STATEMENT && 
							...
							m < leavesM.size()-1 && 
							(Integer)leavesM.get(m+1).getProperty(TreeMappingConstants.propertyStatus) == TreeMappingConstants.STATUS_UNCHANGED)
						hasNeighbor = true;
					if(!hasNeighbor)
					{
						if(n > 0 && (Integer)leavesN.get(n-1).getProperty(TreeMappingConstants.propertyStatus) == TreeMappingConstants.STATUS_UNCHANGED)
							hasNeighbor = true;
						...
						if(nodeTypeN != ASTNode.RETURN_STATEMENT && nodeTypeN != ASTNode.BREAK_STATEMENT && nodeTypeN != ASTNode.CONTINUE_STATEMENT &&
								...
								n < leavesN.size()-1 && 
								(Integer)leavesN.get(n+1).getProperty(TreeMappingConstants.propertyStatus) == TreeMappingConstants.STATUS_UNCHANGED)
							hasNeighbor = true;
					}
					if(!hasNeighbor)
					{
						leafM.setProperty(TreeMappingConstants.propertyStatus, TreeMappingConstants.STATUS_FULLY_CHANGED);
						leafN.setProperty(TreeMappingConstants.propertyStatus, TreeMappingConstants.STATUS_FULLY_CHANGED);
					}
				}
				else if(m < leavesM.size() || n < leavesN.size())
				{
					...
					{
						leafM = leavesM.get(m);
						String pidM = (String)leafM.getProperty(TreeMappingConstants.propertyLocationID);
						System.out.println("\n\n");
						System.out.println(pidM + "@" + leafM.getProperty(TreeMappingConstants.propertyStartLine));
						System.out.println(leafM);
					...
					{
						leafN = leavesN.get(n);
						String pidN = (String)leafN.getProperty(TreeMappingConstants.propertyLocationID);
						System.out.println("\n\n");
						System.out.println(pidN + "@" + leafN.getProperty(TreeMappingConstants.propertyStartLine));
						System.out.println(leafN);
					}
					return -1;
				}
			...
			if(lenM > 0 && lenN > 0)
			{
				...
				for(int i = 0; i < lcsM.size(); i++)
				{
					...
					if(cM - pM == 1 && cN - pN > 1)
						...
						{
							leavesN.get(j).setProperty(TreeMappingConstants.propertyStatus, TreeMappingConstants.STATUS_ADDED);
					...
					if(cN - pN == 1 && cM - pM > 1)
						...
						{
							leavesM.get(j).setProperty(TreeMappingConstants.propertyStatus, TreeMappingConstants.STATUS_DELETED);
	...
	private void mapDeletion(ASTNode nodeM)
	{
		...
		if(!childrenM.isEmpty())
		{
			...
			if(nodesN.isEmpty())
			{
				nodeM.setProperty(TreeMappingConstants.propertyStatus, TreeMappingConstants.STATUS_DELETED);
	...
	private void mapAddition(ASTNode nodeN)
	{
		...
		if(!childrenN.isEmpty())
		{
			...
			if(nodesM.isEmpty())
			{
				nodeN.setProperty(TreeMappingConstants.propertyStatus, TreeMappingConstants.STATUS_ADDED);
	...
	private void mapMoving(ASTNode rootM)
	{
		...
		if(rootN != null)
		{
			...
			if (pM != null && pN != null && pN != pM.getProperty(propertyRevisionMap)) {
				...
				if (pM != null)
					pStatusM = (Integer) pM.getProperty(TreeMappingConstants.propertyStatus);
				if (pN != null)
					pStatusN = (Integer) pN.getProperty(TreeMappingConstants.propertyStatus);
				if (pStatusM != TreeMappingConstants.STATUS_DELETED && pStatusN != TreeMappingConstants.STATUS_ADDED) {
					rootM.setProperty(TreeMappingConstants.propertyStatus, TreeMappingConstants.STATUS_MOVED);
					rootN.setProperty(TreeMappingConstants.propertyStatus, TreeMappingConstants.STATUS_MOVED);
				}
			}
			...
			if (deletedChildren.size() * addedChildren.size() <= MAX_NUM_PAIR_COMPARISONS) {	// TRICK avoid large pair-wise comparison
				...
				while(!similarityPairs.isEmpty())
				{
					...
					node1.setProperty(TreeMappingConstants.propertyStatus, TreeMappingConstants.STATUS_MOVED);
					node2.setProperty(propertyRevisionMap, node1);
					node2.setProperty(TreeMappingConstants.propertyStatus, TreeMappingConstants.STATUS_MOVED);
					if (childrenM.size() == 1 || childrenN.size() == 1) {
						node1.setProperty(TreeMappingConstants.propertyStatus, TreeMappingConstants.STATUS_PARTLY_CHANGED);
						node2.setProperty(TreeMappingConstants.propertyStatus, TreeMappingConstants.STATUS_PARTLY_CHANGED);
					}
		...
		{
			rootM.setProperty(TreeMappingConstants.propertyStatus, TreeMappingConstants.STATUS_DELETED);
		}
		for(ASTNode childM : childrenM)
			mapMoving(childM);
	}
	private void markAddition(ASTNode rootN)
	{
		if(rootN.getProperty(propertyRevisionMap) == null)
			rootN.setProperty(TreeMappingConstants.propertyStatus, TreeMappingConstants.STATUS_ADDED);
	...
	private void mapRelabeling(ASTNode node1, HashMap<ASTNode, ArrayList<ASTNode>> tree1)
	{
		...
		if(children.isEmpty())
		{
			...
			if(node2 != null)
				if(!node1.toString().equals(node2.toString()))
				{
					node1.setProperty(TreeMappingConstants.propertyStatus, TreeMappingConstants.STATUS_RELABELED);
					node2.setProperty(TreeMappingConstants.propertyStatus, TreeMappingConstants.STATUS_RELABELED);
				}
		}
		else
		{
	...
	private boolean checkUnchange(ASTNode node, HashMap<ASTNode, ArrayList<ASTNode>> tree)
	{
		...
		boolean unchanged = status <= TreeMappingConstants.STATUS_FULLY_CHANGED;
		...
		if (unchanged)
			node.setProperty(TreeMappingConstants.propertyStatus, TreeMappingConstants.STATUS_UNCHANGED);
	 ...
	 */
	private int mapUnchange(ASTNode node, HashMap<ASTNode, ArrayList<ASTNode>> tree)
	{
		int numOfChangeNodes = 0;
		...
		if(mappedNode != null)
		{
			int mappedStatus = (Integer)mappedNode.getProperty(TreeMappingConstants.propertyStatus);
			if(status == TreeMappingConstants.STATUS_UNCHANGED && mappedStatus != TreeMappingConstants.STATUS_UNCHANGED) {
				node.setProperty(TreeMappingConstants.propertyStatus, TreeMappingConstants.STATUS_PARTLY_CHANGED);
			}
		}
		ArrayList<ASTNode> children = tree.get(node);
		for(ASTNode child : children)
		{
			numOfChangeNodes += mapUnchange(child, tree);
		}
		status = (Integer)node.getProperty(TreeMappingConstants.propertyStatus);
		ASTNode p = getParent(node);
		if (status != TreeMappingConstants.STATUS_UNCHANGED && p != null) {
			int pStatus = (Integer) p.getProperty(TreeMappingConstants.propertyStatus);
			if (pStatus == TreeMappingConstants.STATUS_UNCHANGED) {
				p.setProperty(TreeMappingConstants.propertyStatus, TreeMappingConstants.STATUS_PARTLY_CHANGED);
			}
		}
		return numOfChangeNodes;
	}
	/**
	 * 
	 */
	...
	public String printTree(HashMap<ASTNode, ArrayList<ASTNode>> tree, ASTNode root)
	{
		...
		if(status > TreeMappingConstants.STATUS_FULLY_CHANGED)
		{
			...
			buf.append(root.getProperty(TreeMappingConstants.propertyLocationID) + "@" + 
					root.getProperty(TreeMappingConstants.propertyStartLine) + ": " + getStatus(status) + "\r\n");
			buf.append(root + "\r\n");
			if(simNode != null)
			{
				buf.append("&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&\r\n");
	...
	{
		switch(status) {
		case TreeMappingConstants.STATUS_ADDED: return "ADDED";
		case TreeMappingConstants.STATUS_DELETED: return "DELETED";
		case TreeMappingConstants.STATUS_FULLY_CHANGED: return "FULLY_CHANGED";
		case TreeMappingConstants.STATUS_MOVED: return "MOVED";
		case TreeMappingConstants.STATUS_PARTLY_CHANGED: return "PARTLY_CHANGED";
		case TreeMappingConstants.STATUS_RELABELED: return "RELABELED";
		case TreeMappingConstants.STATUS_UNCHANGED: return "UNCHANGED";
		}
		return "NOTHING_ELSE";
	}
}
