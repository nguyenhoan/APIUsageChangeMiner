 ...
 * @since 2.0
 */
public class ASTAllNamesFlattener extends ASTVisitor {
	public static final String PROPERTY_SRC = "src";
	public static final String PROPERTY_HEIGHT = "height";
	public static final String PREFIX_VARIABLE = "lv|";
	...
	public void postVisit(ASTNode node) {
		...
		for (int i = size; i < this.tokens.size(); i++) {
			...
			if (token.startsWith(PREFIX_VARIABLE)) {
				String name = token.substring(PREFIX_VARIABLE.length());
	...
	public boolean visit(BooleanLiteral node) {
		...
		this.tokens.add(PREFIX_LITERAL + node.booleanValue() + "|" + node.booleanValue());
		return false;
	}
	...
	public boolean visit(FieldAccess node) {
		...
		this.tokens.add(PREFIX_VARIABLE + node.getName().getIdentifier());
		return false;
	}
...
//		this.tokens.add("\n */\n");//$NON-NLS-1$
	...
	public boolean visit(PrefixExpression node) {
		...
		else if (node.getOperator() == PrefixExpression.Operator.DECREMENT) {
			node.getOperand().accept(this);
			this.tokens.add("+");
			this.tokens.add("1");
		}
	...
	public boolean visit(QualifiedName node) {
		this.tokens.add(PREFIX_VARIABLE + node.getQualifier().toString());
		this.tokens.add(".");//$NON-NLS-1$
		this.tokens.add(PREFIX_VARIABLE + node.getName().getIdentifier());
		return false;
	}
	...
	public boolean visit(SimpleName node) {
		...
		name = PREFIX_VARIABLE + node.getIdentifier();
	...
	public boolean visit(SimpleType node) {
		this.tokens.add(node.getName().toString());
		return false;
	}
