...
public class CClass extends ChangeEntity {
	...
	private ArrayList<CEnumConstant> enumConstants = new ArrayList<CEnumConstant>();
	...
	public CClass(CFile cFile, TypeDeclaration type, CClass outterClass) {
		...
		{
			String name = JavaASTUtil.getType(type.getSuperclassType());
		...
		if (type.superInterfaceTypes() != null)
		{
			...
			{
				String name = JavaASTUtil.getType((org.eclipse.jdt.core.dom.Type) type.superInterfaceTypes().get(i));
				superClassNames.add(name);
				/*if (type.isInterface())
					extendedClassNames.add(name);*/
			}
		}
		//System.out.println("\t\t\tSuper: " + new ArrayList<String>(this.superClassNames));
		if (type.getFields() != null)
		{
			for (FieldDeclaration field : type.getFields())
			{
				...
				{
					VariableDeclarationFragment fragment = (VariableDeclarationFragment) field.fragments().get(i);
					String fieldType = JavaASTUtil.getType(field.getType());
		...
		if (type.getMethods() != null)
		{
			for (MethodDeclaration method : type.getMethods()) {
				...
				if (md.getBody() != null && md.getBody().statements().size() <= CMethod.MAX_NUM_STATEMENTS) // TRICK limit size of method
		...
		if (type.bodyDeclarations() != null) {
			...
			for (int i = 0; i < type.bodyDeclarations().size(); i++) {
				...
				if (type.bodyDeclarations().get(i) instanceof Initializer) {
					initializers.add(new CInitializer(this, staticId, (Initializer)(type.bodyDeclarations().get(i))));
					staticId++;
				}
				else if (type.bodyDeclarations().get(i) instanceof EnumDeclaration) {
					innerClasses.add(new CClass(this.cFile, (EnumDeclaration)(type.bodyDeclarations().get(i)), this));
				}
				else if (nodeType != ASTNode.TYPE_DECLARATION && nodeType != ASTNode.FIELD_DECLARATION && nodeType != ASTNode.METHOD_DECLARATION)
				{
					//System.out.println("Info: not supported " + type.bodyDeclarations().get(i).getClass().getSimpleName() + " in type declaration!!!");
				}
			}
		}
		/*if (outterClass == null)
	...
	public CClass(CFile cFile, EnumDeclaration type, CClass outterClass) {
		this.cFile = cFile;
		this.outterClass = outterClass;
		this.modifiers = type.getModifiers();
		this.simpleName = type.getName().getIdentifier();
		this.declaration = type;
		for (int i = 0; i < type.enumConstants().size(); i++) {
			EnumConstantDeclaration enc = (EnumConstantDeclaration) type.enumConstants().get(i);
			enumConstants.add(new CEnumConstant(this, enc));
			for (int j = 0; j < enc.arguments().size(); j++) {
				Expression expr = (Expression) enc.arguments().get(j);
				expr.toString();
			}
		}
		//System.out.println("\t\t<Class> " + simpleName);
		...
		for (int j = 0; j < type.bodyDeclarations().size(); j++)
		{
			...
			else if (nodeType == ASTNode.METHOD_DECLARATION)
			{
				...
				innerClasses.add(new CClass(cFile, (TypeDeclaration) dec, this));
			}
			else if (nodeType == ASTNode.ENUM_DECLARATION) {
				innerClasses.add(new CClass(cFile, (EnumDeclaration) dec, this));
			}
			else
			{
	...
	public CClass(CFile cFile, AnnotationTypeDeclaration type, CClass outterClass) {
		this.cFile = cFile;
		this.outterClass = outterClass;
		this.modifiers = type.getModifiers();
		this.simpleName = type.getName().getIdentifier();
		...
		for (int j = 0; j < type.bodyDeclarations().size(); j++)
		{
			...
			{
				//System.out.println("Info: not supported " + dec.getClass().getSimpleName() + " in annotation type declaration!!!");
			}
		}
		//System.out.println("\t\t</Class>");
	}
	...
	public String getQualName() {
		String prefix = this.cFile.getPackageName();
		if (this.outterClass != null)
			prefix = this.outterClass.getQualName();
		return prefix + "." + this.simpleName;
	}

	public String getFullQualName() {
		return this.cFile.getPath() + "." + getQualName();
	}
	...
	/*public HashSet<String> getExtendedClassNames() {
	...
	public ArrayList<CEnumConstant> getEnumConstants() {
	...
	public void buildDependencies() {
		/*for (CField cf : this.fields)
			cf.buildDependencies();*/
		for (CMethod cm : this.methods)
				cm.buildDependencies();
		/*for (CInitializer ci : this.initializers)
			ci.buildDependencies();*/
		for (CClass cc : this.innerClasses)
	...
	public double computeSimilarity(CClass otherClass, boolean inMapped) {
		...
		HashSet<CField> fieldsM = new HashSet<CField>(this.fields), fieldsN = new HashSet<CField>(otherClass.getFields());
		HashSet<CField> mappedFieldsM = new HashSet<CField>(), mappedFieldsN = new HashSet<CField>();
		HashSet<CInitializer> initsM = new HashSet<CInitializer>(this.initializers), initsN = new HashSet<CInitializer>(otherClass.getInitializers());
		HashSet<CInitializer> mappedInitsM = new HashSet<CInitializer>(), mappedInitsN = new HashSet<CInitializer>();
		HashSet<CEnumConstant> constsM = new HashSet<CEnumConstant>(this.enumConstants), constsN = new HashSet<CEnumConstant>(otherClass.enumConstants);
		HashSet<CEnumConstant> mappedConstsM = new HashSet<CEnumConstant>(), mappedConstsN = new HashSet<CEnumConstant>();
		
		double[] size = CMethod.mapAll(methodsM, methodsN, mappedMethodsM, mappedMethodsN, inMapped);
		commonSize += size[0]; totalSize += size[1];
		...
		size = CEnumConstant.mapAll(constsM, constsN, mappedConstsM, mappedConstsN);
		commonSize += size[0]; totalSize += size[1];
		
		// map inner classes
		HashSet<CClass> mappedInnersM = new HashSet<CClass>(), mappedInnersN = new HashSet<CClass>();
		mapAll(new HashSet<CClass>(this.innerClasses), new HashSet<CClass>(otherClass.getInnerClasses(false)), mappedInnersM, mappedInnersN);
	...
	public static void setMap(CClass classM, CClass classN) {
		/*if (classM.getMappedClass() != null)
			classM.getMappedClass().setMappedClass(null);
		if (classN.getMappedClass() != null)
	...
	public static void mapAll(HashSet<CClass> classesM, HashSet<CClass> classesN,
							HashSet<CClass> mappedClassesM, HashSet<CClass> mappedClassesN) {
		...
		HashSet<String> interNames = new HashSet<String>(classWithNameM.keySet());
		interNames.retainAll(classWithNameN.keySet());
		for (String name : interNames)
		{
			CClass ccM = classWithNameM.get(name), ccN = classWithNameN.get(name);
			setMap(ccM, ccN);
			mappedClassesM.add(ccM); mappedClassesN.add(ccN);
			classesM.remove(ccM); classesN.remove(ccN);
		}
	...
	public void deriveChanges() {
		...
		boolean same = this.simpleName.equals(ccN.getSimpleName()) && this.modifiers == ccN.getModifiers() &&
				this.annotation.equals(ccN.getAnnotation()) && this.superClassNames.equals(ccN.getSuperClassNames()) /*&&
				this.outterClass.getMappedClass() == ccN.getOutterClass()*/;
		if (same) {
			for (CField cf : fields) {
		...
		if (same) {
			for (CEnumConstant ce : enumConstants) {
				if (ce.getCType() != Type.Unchanged) {
					same = false;
					break;
				}
			}
		...
		if (same) {
			for (CEnumConstant ce : ccN.enumConstants) {
				if (ce.getCType() != Type.Unchanged) {
					same = false;
					break;
				}
	...
	public void printChanges(PrintStream ps) {
		if (getCType() != Type.Unchanged) {
			ps.println("\t\tClass: " + getSimpleName() + " --> " + (this.mappedClass == null ? "null" : this.mappedClass.getSimpleName()));
