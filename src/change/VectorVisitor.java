...
public class VectorVisitor extends ASTVisitor {
	...
	static {
		...
		for(byte i = 0; i < indexer.length; i++) {
			...
			else
			{
				...
				case ASTNode.TYPE_DECLARATION: indexer[i] = ClassFragment; break;
				case ASTNode.METHOD_DECLARATION: indexer[i] = MethodFragment; break;
				//case ASTNode.BLOCK: indexer[i] = Fragment.BlockFragment; break;
				case ASTNode.DO_STATEMENT: indexer[i] = LoopStatementFragment; break;
				case ASTNode.FOR_STATEMENT: indexer[i] = LoopStatementFragment; break;
				case ASTNode.ENHANCED_FOR_STATEMENT: indexer[i] = LoopStatementFragment; break;
				case ASTNode.WHILE_STATEMENT: indexer[i] = LoopStatementFragment; break;
				case ASTNode.IF_STATEMENT: indexer[i] = IfStatementFragment; break;
				//case ASTNode.SWITCH_CASE: indexer[i] = Fragment.IfStatementFragment; break;
				case ASTNode.SWITCH_STATEMENT: indexer[i] = SwitchStatementFragment; break;
				
				case ASTNode.CONDITIONAL_EXPRESSION: indexer[i] = Expression; break;
				case ASTNode.INFIX_EXPRESSION: indexer[i] = Expression; break;
				case ASTNode.POSTFIX_EXPRESSION: indexer[i] = Expression; break;
				case ASTNode.PREFIX_EXPRESSION: indexer[i] = Expression; break;
				case ASTNode.PARENTHESIZED_EXPRESSION: indexer[i] = Expression; break;
				case ASTNode.INSTANCEOF_EXPRESSION: indexer[i] = Expression; break;
				
				case ASTNode.METHOD_INVOCATION: indexer[i] = MethodState; break;
				case ASTNode.SUPER_METHOD_INVOCATION: indexer[i] = MethodState; break;
				...
				case ASTNode.VARIABLE_DECLARATION_STATEMENT: indexer[i] = DeclarationState; break;
				case ASTNode.FIELD_DECLARATION: indexer[i] = DeclarationState; break;
				
				case ASTNode.SIMPLE_NAME: indexer[i] = SimpleName; break;
				case ASTNode.BOOLEAN_LITERAL: indexer[i] = Literal; break;
				case ASTNode.CHARACTER_LITERAL: indexer[i] = Literal; break;
				case ASTNode.STRING_LITERAL: indexer[i] = Literal; break;
				case ASTNode.NUMBER_LITERAL: indexer[i] = Literal; break;
				
				case ASTNode.ARRAY_ACCESS: indexer[i] = ArrayState; break;
				case ASTNode.ARRAY_CREATION: indexer[i] = ArrayState; break;
				case ASTNode.ARRAY_INITIALIZER: indexer[i] = ArrayState; break;
				case ASTNode.ARRAY_TYPE: indexer[i] = ArrayState; break;
				
				case ASTNode.ASSERT_STATEMENT: indexer[i] = AssertState; break;
				case ASTNode.ASSIGNMENT: indexer[i] = AssignState; break;
				
				case ASTNode.MEMBER_REF: indexer[i] = OtherFragments; break;
				case ASTNode.METHOD_REF: indexer[i] = OtherFragments; break;
				case ASTNode.METHOD_REF_PARAMETER: indexer[i] = OtherFragments; break;
				case ASTNode.PRIMITIVE_TYPE: indexer[i] = OtherFragments; break;
				case ASTNode.QUALIFIED_NAME: indexer[i] = OtherFragments; break;
				case ASTNode.TAG_ELEMENT: indexer[i] = OtherFragments; break;
				case ASTNode.TEXT_ELEMENT: indexer[i] = OtherFragments; break;
				case ASTNode.TYPE_PARAMETER: indexer[i] = OtherFragments; break;
				default: indexer[i] = OtherStatementFragment; break;
				}
			}
		}
    }
    ...
    private void buildFragment(ASTNode node) {
		...
		if((//nodeType == ASTNode.TYPE_DECLARATION || 
				...
				(node.getParent() != null && node.getParent().getParent() != null && node.getParent().getParent().getNodeType() == ASTNode.FIELD_DECLARATION) ||
				(nodeType == ASTNode.ENUM_CONSTANT_DECLARATION) ||
				(node.getNodeType() == ASTNode.INITIALIZER && (node.getParent().getNodeType() == ASTNode.TYPE_DECLARATION || node.getParent().getParent().getNodeType() == ASTNode.TYPE_DECLARATION))
				) 
			...
			)
		{
			...
			if(nodeType == ASTNode.TYPE_DECLARATION)
			{
				...
				else if(node.getParent().getParent().getNodeType() == ASTNode.COMPILATION_UNIT)
					name = ((TypeDeclaration)node.getParent()).getName().toString() 
							+ Util.separatorInnerClass + ((TypeDeclaration)node).getName().toString();
			}
